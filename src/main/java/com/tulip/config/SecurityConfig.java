...
public class SecurityConfig {
    ...
    // Spring Security sẽ tự động sử dụng UserDetailsService bean để cấu hình
    // authentication
    // Field này được giữ lại để đảm bảo bean được tạo trước khi SecurityConfig được
    // khởi tạo
    ...
    public AuthenticationManager authenticationManager(AuthenticationConfiguration config) throws Exception {
        // Spring Security sẽ tự động cấu hình DaoAuthenticationProvider từ
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        .requestMatchers("/products/**", "/product/**", "/trending", "/sale", "/about", "/contact")
                        .permitAll()
                        .requestMatchers("/v1/api/store/**").permitAll()
                        .requestMatchers("/v1/api/auth/login", "/v1/api/auth/register", "/v1/api/auth/forgot-password",
                                "/v1/api/auth/reset-password", "/v1/api/auth/resend-otp", "/v1/api/auth/verify-email")
                        .permitAll()
                        ...
                        .requestMatchers("/v1/api/vnpay/create-payment", "/v1/api/momo/create-payment",
                        ...
                        .requestMatchers("/cart", "/checkout", "/checkout/**", "/order-success", "/orders",
                                "/orders/**")
                        .authenticated()
                        .anyRequest().authenticated())
                // Xử lý exception: Nếu đã authenticated nhưng không có quyền hoặc URL không tồn
                // tại
                .exceptionHandling(exceptions -> exceptions
                        .authenticationEntryPoint((request, response, authException) -> {
                            ...
                            if (isAnonymous) {
                                ...
                                response.sendRedirect("/login?redirect=" + java.net.URLEncoder.encode(redirectUrl,
                ...
                .logout(logout -> logout
                        ...
                        .permitAll())
                // Spring Security sẽ tự động sử dụng UserDetailsService và PasswordEncoder
                // beans để cấu hình authentication
